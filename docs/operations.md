...
`copper-model-feature-dock` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for feature workflows, centered on diagnostic reporting, negative fixtures, and human-readable error snapshots.
